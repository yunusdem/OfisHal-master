...
{
    internal class TohalKullaniciConfiguration : EntityTypeConfiguration<TohalKullanici>
    {
        public TohalKullaniciConfiguration()
        {
            HasKey(e => e.KullaniciId);

            ToTable("TOHAL_KULLANICI");
            ...
            Property(e => e.BaktigiAlsFaturaId).HasColumnName("BAKTIGI_ALS_FATURA_ID");

            Property(e => e.BaktigiSatFaturaId).HasColumnName("BAKTIGI_SAT_FATURA_ID");
            ...
            HasOptional(d => d.EFaturaGondericiBirimi)
                .WithMany(p => p.TohalKullaniciEFaturaGondericiBirimis)
                .HasForeignKey(d => d.EFaturaGondericiBirimiId);

            HasOptional(d => d.EIrsaliyeGondericiBirimi)
                .WithMany(p => p.TohalKullaniciEIrsaliyeGondericiBirimis)
                .HasForeignKey(d => d.EIrsaliyeGondericiBirimiId);

            HasOptional(d => d.KaldigiAlsFatura)
                .WithMany(p => p.TohalKullaniciKaldigiAlsFaturas)
                .HasForeignKey(d => d.KaldigiAlsFaturaId);

            HasOptional(d => d.KaldigiSatFatura)
                .WithMany(p => p.TohalKullaniciKaldigiSatFaturas)
                .HasForeignKey(d => d.KaldigiSatFaturaId);
        }
    }
}
